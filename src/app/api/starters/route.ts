...
export async function POST(req: NextRequest) {
  try {
    ...
    // (o resto da manutenção é criado pelo cron diário). Se isto falhar
    // (ex: credenciais Google inválidas), não deve impedir a criação da massa mãe.
    try {
      ...
      console.error('Falha ao criar eventos no Google Calendar:', googleErr);
    }

    return NextResponse.json({ starter });
  } catch (err) {
    console.error('Erro ao criar starter:', err);
    const mensagem = err instanceof Error ? err.message : 'Erro desconhecido no servidor';
